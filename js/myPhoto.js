$(document).ready(function() {
    ...
    function showPage(res, currentPage) {
        ...
        if (totalCount === 0) {
            return '<h1>你还没有上传任何照片，快点击个人中心下拉菜单中的上传条目，进行上传吧！</h1>';
        }
        let mark = (currentPage - 1) * pageSize;
        let left = totalCount - mark;
        let str = '<div class="title"><h5>我的照片</h5></div>\n' +
            '    <hr>';
        if (left >= pageSize) {
            for (let i = 0; i < pageSize; i++) {
                ...
                str += '<div class="item-content item-content'+key+'">\n' +
                    ...
                    '        </div>\n' +
                    '<a href="upload.html?ImageID=' + res[key].ImageID + '"><input type="button" value="修改" class="modify"></a>\n' +
